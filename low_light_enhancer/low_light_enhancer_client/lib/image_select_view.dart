...
import 'package:low_light_enhancer_client/constants/string_constants.dart';
import 'package:low_light_enhancer_client/widgets/app_memory_image.dart';
import 'package:low_light_enhancer_client/widgets/app_shimmer.dart';
import 'package:low_light_enhancer_client/widgets/app_text.dart';
  ...
  Center outputImageBuilder() {
    return Center(
      child: ValueListenableBuilder(
        valueListenable: enhanceImageState,
        builder: (context, state, _) {
          Widget widget;

          if (state.status == EnhanceImageFetchStatus.initial) {
            widget = const SizedBox.shrink();
          } else if (state.status == EnhanceImageFetchStatus.error) {
            widget = const AppText(
              text: StringConstants.networkFetchErrorText,
            );
          } else if (state.status == EnhanceImageFetchStatus.loading) {
